...
- You can override default parameters (defined in `variables.tf`) either with the CLI <!-- NS: this part is not clear – we already provide `terraform.tfvars` in the repository, should it be deleted if vars are to be set in CLI? --> or by adjusting a file called `terraform.tfvars` in the module's root directory
...
The following steps were tested on Ubuntu 20.04 but supposed to be valid for other Linux distributions without significant modification.
 ...
 1. Get TF code for Database Lab:
    ...
    git checkout ci_observer_deployment_with_TF
    ```
